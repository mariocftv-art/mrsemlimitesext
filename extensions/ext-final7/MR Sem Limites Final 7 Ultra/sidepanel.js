// ============================================================
// ILIMITADO MR Extension - v4.4.7
// Thin Client Shell + MR-ULTRA Backend
// ============================================================

const EXTENSION_VERSION = '5.1.0-NEON-NOIR'; // versão exibida para o usuário
...
async function generateHWID() {
  ...
  console.log('[BLACK & GOLD] HWID gerado via sidepanel fallback:', deviceId);
  return cachedHwid;
}
...
async function getAuthData() {
  try {
    ...
    // Tenta o token do storage MR-ULTRA (capturado pelo inject.js)
  ...
  window.addEventListener('message', async (event) => {
    ...
    try {
      switch (command) {
        ...
        // ---- Send message (via backend MR-ULTRA) ----
...
async function fetchRemoteUiHtml() {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    if (attempt > 0) {
      console.warn(`[BLACK & GOLD] fetchRemoteUiHtml retry ${attempt}/2 após ${RETRY_DELAYS[attempt]}ms...`);
    ...
    try {
      ...
      lastError = e;
      console.error(`[BLACK & GOLD] fetchRemoteUiHtml erro tentativa ${attempt + 1}:`, e?.message || e);
  ...
  // MR SEM LIMITES FINAL 7 ULTRA VELOZ — Premium CSS + Chat layout fix
  ...
  var mrf7ucss = `<style id="mrf7u-btns">
@keyframes mrf7u-glow{0%,100%{box-shadow:0 0 8px rgba(212,169,74,.3)}50%{box-shadow:0 0 18px rgba(212,169,74,.55)}}
...
/* Enhance/Otimizar com IA — POSITION ABSOLUTE: sai do fluxo, flutua acima */
  ...
  background:rgba(212,169,74,.08)!important;
  border:1px solid rgba(212,169,74,.25)!important;
...
#enhanceBtn:hover{background:rgba(212,169,74,.18)!important;border-color:rgba(212,169,74,.4)!important;box-shadow:0 3px 14px rgba(212,169,74,.2)!important;transform:translateY(-1px)!important}
...
/* Send button */
#sendBtn,button.send-btn{background:linear-gradient(135deg,#8f6b1e,#f5dc8c)!important;color:#fff!important;border:none!important;border-radius:12px!important;width:38px!important;height:38px!important;min-width:38px!important;display:flex!important;align-items:center!important;justify-content:center!important;cursor:pointer!important;padding:0!important;flex-shrink:0!important;box-shadow:0 4px 18px rgba(212,169,74,.45)!important;transition:all .25s cubic-bezier(.4,0,.2,1)!important}
#sendBtn:hover{transform:scale(1.08)!important;box-shadow:0 6px 24px rgba(212,169,74,.6)!important}
#sendBtn:active{transform:scale(.95)!important}
#sendBtn:disabled{background:rgba(212,169,74,.12)!important;box-shadow:none!important;transform:none!important;cursor:not-allowed!important}

/* Attach button */
#attachBtn,button#attachBtn{background:rgba(212,169,74,.08)!important;border:1.5px solid rgba(212,169,74,.28)!important;color:#f5dc8c!important;border-radius:12px!important;width:38px!important;height:38px!important;min-width:38px!important;display:flex!important;align-items:center!important;justify-content:center!important;cursor:pointer!important;padding:0!important;flex-shrink:0!important;transition:all .25s cubic-bezier(.4,0,.2,1)!important;backdrop-filter:blur(4px)!important}
#attachBtn:hover{background:rgba(212,169,74,.18)!important;border-color:rgba(212,169,74,.45)!important;transform:scale(1.06)!important;box-shadow:0 3px 14px rgba(212,169,74,.25)!important}
...
/* Download button */
#downloadBtn{background:rgba(212,169,74,.06)!important;border:1.5px solid rgba(212,169,74,.2)!important;color:#fff2c0!important;border-radius:20px!important;padding:5px 12px!important;font-size:11.5px!important;cursor:pointer!important;transition:all .25s cubic-bezier(.4,0,.2,1)!important;backdrop-filter:blur(4px)!important}
#downloadBtn:hover{background:rgba(212,169,74,.14)!important;border-color:rgba(212,169,74,.38)!important;transform:translateY(-1px)!important}
...
.dl-mode-btn.active{background:linear-gradient(135deg,#8f6b1e,#f5dc8c)!important;color:#fff!important;box-shadow:0 3px 14px rgba(212,169,74,.35)!important}
.dl-mode-btn:hover:not(.active){background:rgba(212,169,74,.1)!important}
...
#tabChat:hover,#tabTemplates:hover{color:#fff2c0!important}

/* Textarea focus glow */
textarea#message:focus{border-color:rgba(212,169,74,.45)!important;box-shadow:0 0 0 3px rgba(212,169,74,.08)!important}
...
.bubble, .message-bubble, [class*="bubble"], [class*="message-content"] {
  ...
  if (/<\/head>/i.test(finalHtml)) {
    finalHtml = finalHtml.replace(/<\/head>/i, mrf7ucss + '</head>');
  } else {
    finalHtml = mrf7ucss + finalHtml;
  }
  return finalHtml;
}
...
Regras: NÃO quebrar funcionalidades, NÃO remover features, NÃO alterar design base. Priorizar adaptação. O sistema deve estar 100% responsivo, sem quebras, com excelente UX mobile e pronto para produção.`
  ...
  console.log('[BLACK & GOLD] Chat direto inicializado — sem iframe');
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Check stored license — suporta formato MR-ULTRA (settings.licenseKey) e legado
  async function checkStoredLicense() {
    // Tenta ler do formato MR-ULTRA primeiro
