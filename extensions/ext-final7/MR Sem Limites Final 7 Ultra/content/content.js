...
(function () {
  ...
  async function init() {
    ...
    console.log('[MRF7U content] init â€” active:', STATE.active, 'license:', STATE.licenseValid ? 'valid' : 'invalid', 'email:', STATE.userEmail || 'none');
  ...
  async function handleTransformRequest(data) {
    ...
    console.log('[MRF7U content] transform request -> lov5',
      'key:', licenseKey ? '***' + licenseKey.slice(-4) : 'MISSING');

    if (!licenseKey) {
      console.error('[MRF7U content] no license key available!');
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.warn('[MRF7U content] lov5 HTTP', res.status, errText.slice(0, 120), '-> pass-through (servidor indisponivel)');
      ...
      console.log('[MRF7U content] lov5 transform:', result.action || 'sem action');
      ...
      console.warn('[MRF7U content] lov5 inacessivel:', e.message, '-> pass-through (sem conexao)');
  ...
  function setupAnexosCleanup() {
    ...
    function hideLeakedComments(root) {
      ...
      for (const bq of blockquotes) {
        ...
        if (!bqText.includes('MRF7U-') && !bqText.includes('Pulse Coding Mode') &&
        ...
        while (prev) {
          ...
          if (pt.includes('[MRF7U-INI]') || pt.includes('[MRF7U-FIM]') ||
              pt.includes('[LVFE-INI]') || pt.includes('[LVFE-FIM]')) {
            prev.style.display = 'none';
          }
          if (pt.includes('[MRF7U-INI]') || pt.includes('[LVFE-INI]')) break;
          prev = prev.previousElementSibling;
        }
        // Esconde markers depois do blockquote
        let next = bq.nextElementSibling;
        if (next && ((next.textContent || '').includes('[MRF7U-FIM]') ||
        ...
        if (!bubble.dataset.pcPulseBadged) {
          ...
          badge.innerHTML = '<span class="pc-pulse-dot">âœ¦</span><span>MR SEM LIMITES FINAL 7 ULTRA IA</span>';
          badge.title = 'Processado pela MR SEM LIMITES FINAL 7 ULTRA';
  ...
  function setupRebrand() {
    ...
    const REPLACEMENT = 'MR SEM LIMITES FINAL 7 ULTRA';
    ...
    function applySpecialText(el) {
      if (el.dataset.mrf7uDone) return;
      el.dataset.mrf7uDone = '1';
      function enforce() {
    ...
    window.addEventListener('message', (e) => {
      ...
      switch (data.type) {
        case 'LOVABLE_INJECT_READY':
          console.log('[MRF7U content] inject.js ready, pushing state');
          announceActive();
          pushConfig();
          pushLicense();
          break;
        ...
        case 'LOVABLE_PROMPT_ENHANCED':
          ...
          showIlSuccessToast('✅ Enviado via Ilimitado MR');
        ...
        case 'LOVABLE_PROMPT_ERROR':
          ...
          console.warn('[MRF7U content] prompt error:', data.status, data.statusText);
        ...
        case 'LOVABLE_TRANSFORM_FAILED':
          ...
          console.warn('[MRF7U content] transform failed (ignorado):', data.error);
          break;
        ...
        <div class="pc-pc-steps">
          ...
          <div class="pc-pc-step">
            ...
            <div class="pc-pc-step-text">
              ...
              <span class="pc-pc-step-detail">PoderÃ¡ ficar <b>grÃ¡tis</b> via MR SEM LIMITES FINAL 7 ULTRA</span>
            </div>
            <span class="pc-pc-step-badge pc-pc-step-pulse">MRF7U</span>
          </div>
        </div>
  ...
  function bindStorageChanges() {
    chrome.storage.onChanged.addListener((changes, area) => {
      ...
      if (!STATE.licenseValid) {
        ...
        console.log('[MRF7U content] Licenca revogada/desconectada - extensao desativada');
      ...
      if (STATE.active !== wasActive) {
        console.log('[MRF7U content] active changed:', STATE.active);
        announceActive();
      }
  ...
  function bindExtensionMessages() {
    chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
      ...
      if (msg?.type === 'TOGGLE_PANEL') {
        console.log('[MRF7U content] TOGGLE_PANEL received from toolbar icon');
        togglePanel();
        return;
      }
  ...
  async function handleFetchWorkspaceId(msg) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.warn('[MRF7U] /user/workspaces falhou:', res.status, detail);
        return { ok: false, status: res.status, error: `HTTP ${res.status}: ${detail}` };
      }
      const data = await res.json().catch(() => null);
      const list = Array.isArray(data?.workspaces) ? data.workspaces : [];
      console.log('[MRF7U] /user/workspaces retornou', list.length, 'workspaces');
      ...
      console.log('[MRF7U] workspace_id escolhido:', wsId, '(name:', chosen.name + ', role:', chosen.membership?.role + ', projects:', chosen.num_projects + ')');
      ...
      console.error('[MRF7U] handleFetchWorkspaceId failed:', e);
  ...
  async function handleCreateProject(msg) {
    ...
    try {
      ...
      console.error('[MRF7U content] create project failed:', e);
      return { ok: false, error: e?.message || String(e) };
    }
  }
  ...
  async function handleSetChatMode(msg) {
    ...
    try {
      ...
      console.error('[MRF7U content] set chat mode failed:', e);
      return { ok: false, error: e?.message || String(e) };
    }
  }
  ...
  async function handleFixAllSecurity(msg) {
    ...
    try {
      ...
      console.error('[MRF7U content] fix all security failed:', e);
      return { ok: false, error: e?.message || String(e) };
    }
  }
  ...
  async function handleGetSecurityData(msg) {
    ...
    try {
      ...
      console.error('[MRF7U content] security data fetch failed:', e);
      return { ok: false, error: e?.message || String(e) };
    }
  }
  ...
  async function handleRunSecurityScan(msg) {
    ...
    try {
      ...
      console.error('[MRF7U content] security scan trigger failed:', e);
      return { ok: false, error: e?.message || String(e) };
    }
  }
  ...
  async function handlePublishProject(msg) {
    ...
    try {
      ...
      console.error('[MRF7U content] publish failed:', e);
      return { ok: false, error: e?.message || String(e) };
    }
  }
    ...
    try {
      ...
      for (const att of attachmentsIn) {
        try {
          ...
          console.error('[MRF7U content] attachment upload failed:', e);
      ...
      if (res.status === 401 || res.status === 403) {
        ...
        if (fresh.token !== msg.token || fresh.sessionId !== (msg.sessionId || '')) {
          console.log('[MRF7U content] auth mudou â€” reenviando via proxy');
      ...
      if (!res.ok && res.status === 400) {
        ...
        for (let r = 0; r < RETRY_DELAYS.length; r++) {
          console.warn('[MRF7U content] 400 transitorio - retry ' + (r + 1) + '/' + RETRY_DELAYS.length + ' em ' + RETRY_DELAYS[r] + 'ms...');
      ...
      console.error('[MRF7U content] try-to-fix failed:', e);
      return { ok: false, error: e?.message || String(e) };
    }
  }
  ...
  async function handleHideLovableBadge(msg) {
    ...
    try {
      ...
      console.error('[MRF7U content] hide badge failed:', e);
      return { ok: false, error: e?.message || String(e) };
    }
  }
  ...
  async function handleDownloadProject(msg) {
    ...
    try {
      ...
      console.error('[MRF7U content] download failed:', e);
      return { ok: false, error: e?.message || String(e) };
    } finally {
  ...
  function setupNativeChatGlow() {
    ...
    style.textContent = `
      @keyframes il-border-gradient {
        0%   { outline: 2px solid #7a5a0a; outline-offset: 1px; filter: drop-shadow(0 0 3px rgba(109,40,217,0.5)); }
        25%  { outline: 2px solid #f5dc8c; outline-offset: 2px; filter: drop-shadow(0 0 5px rgba(212,169,74,0.6)); }
        ...
        100% { outline: 2px solid #7a5a0a; outline-offset: 1px; filter: drop-shadow(0 0 3px rgba(109,40,217,0.5)); }
      }
      ...
      @keyframes il-ball-ring {
        0%, 100% {
          box-shadow: 0 0 0 2px #f5dc8c, 0 0 10px 2px rgba(212,169,74,0.5);
        }
        50% {
          box-shadow: 0 0 0 4px #fff2c0, 0 0 20px 6px rgba(245,220,140,0.55);
        }
      }
      ...
      /* MR SEM LIMITES FINAL 7 ULTRA ATIVO — dentro do chat, canto inferior esquerdo */
      .ilimitado-glow-active::before {
        content: "MR SEM LIMITES FINAL 7 ULTRA ATIVO";
        position: absolute;
        bottom: 6px;
        left: 12px;
        font-size: 9px;
        ...
        color: rgba(245,220,140,0.7);
        pointer-events: none;
        z-index: 11;
        text-shadow: 0 0 6px rgba(212,169,74,0.6);
      }
      .ilimitado-btn-glow svg {
        color: #fff2c0 !important;
        filter: drop-shadow(0 0 4px #f5dc8c) !important;
      }
      /* ---- Floating icon ball ---- */
      #il-float-ball {
        position: fixed;
      ...
      .il-sub-btn {
        ...
        box-shadow: 0 2px 12px rgba(212,169,74,0.3);
      }
      .il-sub-btn svg {
      ...
      .il-sub-btn:hover {
        ...
        box-shadow: 0 4px 20px rgba(212,169,74,0.5);
      }
      .il-sub-btn:active {
...
Isso vai remover a marca d'água do Lovable. Aplique essa alteração agora.`,
    ...
    function ensureFloatBall() {
      ...
      floatBall.innerHTML = `
        <img src="${iconUrl}" alt="Ilimitado MR">
    ...
    setInterval(async () => {
      ...
      if (!licValid) {
        ...
        if (STATE.active || STATE._manuallyActivated) {
          ...
          console.log('[MRF7U content] Licenca invalida - extensao desativada completamente');
        }
        return;
      }
      ensureFloatBall();
    ...
    if (!t) {
      ...
      style.textContent = `
        #il-native-toast {
          ...
          box-shadow: 0 4px 15px rgba(0,0,0,0.5), 0 0 10px rgba(212,169,74,0.4);
  ...
  function togglePanel() {
    console.log('[MRF7U] togglePanel â€” host:', !!STATE.panelHost, 'open:', STATE.panelOpen);
    if (STATE.panelHost) {
  ...
  function createPanel() {
    try {
      ...
      console.log('[MRF7U] createPanel â€” popup:', popupUrl);
      ...
      panel.innerHTML = `
        <div class="panel-header">
          <img class="panel-logo" src="${bannerUrl}" alt="MR SEM LIMITES FINAL 7 ULTRA" />
      ...
      console.log('[MRF7U] panel created âœ“');
    } catch (err) {
      console.error('[MRF7U] createPanel FAILED:', err);
    }
  }
