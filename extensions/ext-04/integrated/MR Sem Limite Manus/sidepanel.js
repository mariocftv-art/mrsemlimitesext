...
const EXTENSION_API_VERSION = '6.3.0';      
console.log(`🚀 MR Sem Limite Manus v${EXTENSION_VERSION} (MRSL) iniciando...`);
  ...
  window.addEventListener('message', async (event) => {
    ...
    try {
      switch (command) {
        ...
        const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
        if (!tab?.id || !/lovable\.dev|lovableproject\.com|manus\.im|manus\.ai/.test(tab.url || '')) {
          error = 'Abra a aba do Manus ou Lovable com o projeto antes de enviar.'; break;
        }
        const injected = await new Promise((resolve) => {
          const timeout = setTimeout(() => resolve({ ok: false, error: 'Timeout: content script não respondeu. Recarregue a página do Manus.' }), 10000);
          chrome.tabs.sendMessage(tab.id, { type: 'TYPE_AND_SEND_IN_LOVABLE', text: msgText }, (resp) => {
            clearTimeout(timeout);
            void chrome.runtime.lastError;
            resolve(resp || { ok: false, error: 'content script não respondeu. Recarregue a página do Manus.' });
          });
        });
        if (injected.ok) {
      ...
      case 'lovable.publish': {
          const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
          if (!tab?.url || !/lovable\.dev|manus\.im|manus\.ai/.test(tab.url)) {
            error = 'Você precisa abrir o Manus ou Lovable!';
            break;
          }
          const pId2 = await getProjectFromActiveTab();
          if (!pId2) { error = 'Abra um projeto no Manus ou Lovable primeiro'; break; }
  ...
  try {
    switch (command) {
      ...
      case 'lovable.sendMessage': {
        ...
        try {
          const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
          if (!tab?.id || !tab?.url || !/lovable\.dev|manus\.im|manus\.ai/.test(tab.url)) {
            error = 'Abra o Manus ou Lovable na aba ativa primeiro.';
            break;
          }
          // INJEÇÃO DIRETA À PROVA DE BALAS
          await chrome.scripting.executeScript({
            target: { tabId: tab.id },
            func: (text, files) => {
              window.postMessage({ type: 'TYPE_AND_SEND_IN_LOVABLE', text, files }, '*');
            },
            args: [msgText, normalized]
          });
          const resp = { ok: true };
          if (resp?.ok) {
      ...
      case 'lovable.publish': {
        const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
        if (!tab?.url || !/lovable\.dev|manus\.im|manus\.ai/.test(tab.url)) { error = 'Você precisa abrir a plataforma!'; break; }
        const pId2 = await getProjectFromActiveTab();
...
async function sendDirectLovableMessage(messageText) {
  ...
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!tab?.id || !tab?.url || !/lovable\.dev|manus\.im|manus\.ai/.test(tab.url)) {
    throw new Error('Abra o Manus ou Lovable na aba ativa primeiro.');
  }
...
async function sendAndReadLovableReply(messageText, opts = {}) {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!tab?.id || !/lovable\.dev|manus\.im|manus\.ai/.test(tab.url || '')) {
    throw new Error('Abra o Manus ou Lovable na aba ativa primeiro.');
  }
