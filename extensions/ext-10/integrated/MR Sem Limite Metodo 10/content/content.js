...
(function () {
  ...
  if (!/(^|\.)(lovable\.dev|manus\.im|manus\.ai)$/i.test(location.hostname)) {
    return;
  }
  ...
  function isProjectPage() {
    if (location.hostname.includes('manus.im') || location.hostname.includes('manus.ai')) return true;
  ...
  function bindExtensionMessages() {
    chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
      ...
      if (msg?.type === 'PING') {
        console.log('[MR OVERDRIVE] PING recebido do sidepanel');
        sendResponse({ ok: true, project: true, pathname: location.pathname, status: 'ready' });
        return true; 
      }
      ...
      if (msg?.type === 'TYPE_AND_SEND_IN_LOVABLE') {
        (async () => {
          try {
            ...
            if (!el) { sendResponse({ ok: false, error: 'campo de chat não encontrado na plataforma' }); return; }
            ...
            if (files.length > 0) {
              try {
                ...
                if (!fileInput) { sendResponse({ ok: false, error: 'campo de anexo não encontrado na plataforma' }); return; }
            ...
            if (text) {
              if (el.tagName === 'TEXTAREA' || el.tagName === 'INPUT') {
                ...
                try {
                  // Injeção via ProseMirror View (se disponível)
                  if (el.view && el.view.dispatch) {
                    const { state } = el.view;
            ...
            const findSendBtn = () => {
              ...
              cand = btns.find(b => /enviar|send|enviar mensagem|send message/i.test((b.getAttribute('aria-label') || '') + ' ' + (b.textContent || '')));
              if (cand) return cand;
              // 2.1) Classes específicas do Manus
              cand = btns.find(b => b.className && typeof b.className === 'string' && (b.className.includes('SendButton') || b.className.includes('send_button')));
              if (cand) return cand;
            ...
            if (textAfterRetry.length > 0 && (!btn || btn.disabled || btn.getAttribute('aria-disabled') === 'true')) {
              sendResponse({ ok: false, error: 'texto digitado, mas botão Enviar não habilitou' });
              return;
            }
...
Isso vai remover a marca d'água do Lovable. Aplique essa alteração agora.`,
    ...
    async function sendSubAction(actionId) {
      ...
      try {
        ...
        let pId = m ? m[1] : null;
        if (!pId && location.hostname.includes('manus.im')) pId = 'manus-active-session';
        if (!pId) { showIlSuccessToast('❌ Abra o Manus ou Lovable primeiro'); return; }
  ...
  function createPanel() {
    try {
      const bannerUrl = chrome.runtime.getURL('icons/icon128.png');
      ...
      panel.innerHTML = `
        <div class="panel-header">
          <img class="panel-logo" src="${bannerUrl}" alt="MR Sem Limite Manus" />
