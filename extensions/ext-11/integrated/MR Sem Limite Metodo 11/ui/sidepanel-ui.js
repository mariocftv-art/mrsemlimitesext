...
/* ============================================================
 * Skills (EXT6) — Habilidades prontas que injetam prompt no chat
 * ============================================================ */
function initSkills() {
  const catsEl = $('#mr6SkillsCats');
  const gridEl = $('#mr6SkillsGrid');
  const searchEl = $('#mr6SkillsSearch');
  if (!catsEl || !gridEl) return;
  ...
  const renderCats = () => {
    catsEl.innerHTML = SKILL_CATEGORIES.map(c =>
      `<button class="mr6-skills-cat${c.id === activeCat ? ' active' : ''}" data-cat="${c.id}"><span>${c.icon}</span>${escapeHtml(c.name)}</button>`
    ).join('');
  };
  ...
  const renderGrid = () => {
    ...
    if (!list.length) {
      gridEl.innerHTML = `<div class="mr6-skills-empty">Nenhuma skill encontrada.</div>`;
      return;
    }
    gridEl.innerHTML = list.map(s => `
      <button class="mr6-skill-card" data-id="${escapeAttr(s.id)}" title="Clique para usar esta skill">
        <div class="mr6-skill-icon">${s.icon}</div>
        <div class="mr6-skill-body">
          <div class="mr6-skill-name">${escapeHtml(s.name)}</div>
          <div class="mr6-skill-desc">${escapeHtml(s.desc)}</div>
        </div>
        <div class="mr6-skill-arrow">→</div>
      </button>
    `).join('');
  };
