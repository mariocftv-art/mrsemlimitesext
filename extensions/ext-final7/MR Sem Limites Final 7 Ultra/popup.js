// ============================================================
// ILIMITADO MR Extension v4.4.7 - Popup (chat direto)
...
// ============ License (via background MR-ULTRA) ============
...
async function validateLicense(licenseKey) {
  try {
    console.log('[MR] Validando licença via background:', licenseKey?.substring(0, 8) + '***');
    ...
    console.error('[MR] Erro ao validar licença:', error);
...
async function sendChatMessage(sessionToken, message, projectId) {
  try {
    ...
    console.error('[MR] Erro ao enviar mensagem:', error);
    return { status: 'error', message: error.message };
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ============ License Check (via background — formato MR-ULTRA) ============
  async function checkStoredLicense() {
    // Lê do formato MR-ULTRA (settings.licenseState) e do legado
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener('click', async () => {
      // Limpa via background (formato MR-ULTRA)
