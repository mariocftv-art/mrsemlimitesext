...
function initDirectChat() {
  ...
  // ── CONTADOR DE PROMPTS (ciclo 24h) ─────────────────────────────────────
  const pcwCount   = document.getElementById('pcwCount');
  const pcwBar     = document.getElementById('pcwBar');
  const pcwSub     = document.getElementById('pcwSub');
  const pcwAlert   = document.getElementById('pcwAlert');
  ...
  function renderPromptCounter(stats) {
    if (!pcwCount) return;
    const now         = Date.now();
    const cycleStart  = stats?.cycleStartAt || 0;
    const elapsed     = cycleStart ? now - cycleStart : 0;

    // Ciclo expirou ou ainda não iniciou
    let count = 0;
    let timeLeftMs = 0;
    if (cycleStart && elapsed < CYCLE_MS) {
      count      = Math.min(stats?.cycleCount || 0, LIMIT);
      timeLeftMs = CYCLE_MS - elapsed;
    }

    const pct   = Math.min((count / LIMIT) * 100, 100);
    const level = count >= 95 ? 'danger' : count >= 80 ? 'warn' : 'ok';

    // Texto e barra
    pcwCount.textContent = `${count} / ${LIMIT}`;
    pcwCount.className   = `pcw-count ${level}`;
    if (pcwBar) {
      pcwBar.style.width  = `${pct}%`;
      pcwBar.className    = `pcw-bar-fill ${level}`;
    }

    // Subtítulo
    if (!cycleStart || elapsed >= CYCLE_MS) {
      pcwSub.textContent = 'Aguardando primeiro envio';
    } else {
      pcwSub.textContent = `Renova em ${formatTimeLeft(timeLeftMs)}`;
    }

    // Alerta perto do limite
    if (pcwAlert) {
      if (count >= 90 && timeLeftMs > 0) {
        pcwAlert.classList.add('visible');
      } else {
        pcwAlert.classList.remove('visible');
      }
    }
  }
  ...
  async function refreshCounter() {
    try {
      ...
      renderPromptCounter(data?.settings?.stats || {});
    } catch (e) {}
  }
  refreshCounter();
  setInterval(refreshCounter, 5000); // atualiza a cada 5s (o sub-segundo vem do timer local abaixo)

  // Timer local de 1s só para atualizar o "renova em Xh Xm Xs" sem bater no storage
  let _lastStats = null;
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg.type === 'STATS_UPDATED') { _lastStats = msg.stats; renderPromptCounter(_lastStats); }
  });
  setInterval(() => { if (_lastStats) renderPromptCounter(_lastStats); }, 1000);
  // ────────────────────────────────────────────────────────────────────────
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (res?.error) {
        ...
        updateStatus('');
        // Sinaliza ao background que o prompt foi enviado com sucesso
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Check stored license — suporta formato LOV-ULTRA (settings.licenseKey) e legado
