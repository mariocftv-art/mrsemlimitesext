...
(function () {
  ...
  async function handleTransformRequest(data) {
    ...
    const castleToken = data.castleToken || null;

    console.log('[PULSE content] transform request → lov4',
      'key:', licenseKey ? '***' + licenseKey.slice(-4) : 'MISSING',
      'email:', email || 'MISSING',
      'castle:', castleToken ? 'present' : 'missing');
    ...
    try {
      const res = await fetch(LOV4_URL, {
        ...
        body: JSON.stringify({
          ...
          body,
          uploadedAssets,
          castle_token: castleToken || body.castle_request_token || null,
        }),
      });
  ...
  async function handleSendTryToFix(msg) {
    ...
    try {
      ...
      if (planMode) {
        ...
        // v7.3.1 — Sem dispatch_mode e source (rastreadores removidos)
        body.message =
          'For the code present, I get the error below.\n\n' +
          'Please think step-by-step in order to resolve it.\n' +
          '```\n' + userText + '\n```\n';
