...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (!_MRF7U_RUNTIME_OK) {
    sendResponse({ ok: false, error: 'runtime invalid' });
    return;
  }
  return _MRF7U_handler(msg, _sender, sendResponse);
});

function _MRF7U_handler(msg, _sender, sendResponse) {
  (async () => {
    try {
      switch (msg.type) {
        ...
        case 'SEND_MESSAGE_PROXY': {
          ...
          // Mesmo gateway canônico usado para ativar a licença e pela EXT7.
          // O endpoint legado lov5 consultava outro banco e rejeitava a chave
          // logo após uma ativação válida, causando o retorno à tela de licença.
          ...
          for (let attempt = 0; attempt <= 3; attempt++) {
            ...
            try {
              const payload = {
                ...
                // Licença — o servidor valida antes de enviar (gate server-side)
              ...
              if (proxyResp.status === 400) {
                console.warn(`[MRF7U bg] 400 attempt ${attempt + 1}/4, retrying...`);
...
async function checkLocalExpiry() {
  ...
  if (now >= expiresMs) {
    console.log('[MRF7U bg] license expired locally, revoking');
    const expired = {
      ...emptyLicenseState(),
      status: 'expired',
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  if (!s.deviceId) {
    ...
    console.log('[MR] HWID gerado:', deviceId);
  }
...
chrome.runtime.onStartup?.addListener(async () => {
  ...
  if (!s.deviceId) {
    const deviceId = crypto.randomUUID();
    await setSettings({ deviceId });
    console.log('[MR BLACK & GOLD] HWID gerado no startup:', deviceId);
  }
});
...
chrome.alarms?.onAlarm.addListener(async (alarm) => {
  ...
  try {
    ...
    console.warn('[MR BLACK & GOLD] Erro transitório no polling de licença:', e?.message || e);
    return;
  }
  if (!state) return;
  ...
  if (state.status === 'transient' || state.status === 'device_mismatch') {
    console.warn('[MR] polling não-conclusivo (' + state.status + ') — mantendo licença:', state.error);
    return;
  }
  ...
  updateBadge(updated);
  broadcastLicenseRevoked();
  console.log('[MR] licença revogada/expirada no polling:', state.status, state.error);
});
...
async function doLicenseLogout(errMsg) {
  try {
    ...
    updateBadge(updated);
    broadcastLicenseRevoked();
    console.log('[MR] logout forçado pelo servidor (licença inválida):', errMsg);
  } catch (e) {
    console.warn('[MR] falha ao deslogar após rejeição do servidor:', e?.message || e);
  }
}
...
try {
  chrome.webRequest.onSendHeaders.addListener(
    (details) => {
      ...
      if (apiToken) {
        ...
        console.log('[MR bg] token capturado via webRequest');
      }
    },
  ...
  console.warn('[MR bg] onSendHeaders indisponível:', e?.message);
}
...
try {
  ...
  console.log('[MR bg] onBeforeRequest ativo — captura payload nativo do chat');
} catch (e) {
  console.warn('[MR bg] onBeforeRequest indisponível:', e?.message);
}
