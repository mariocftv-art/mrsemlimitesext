    ...
    try {
      switch (msg.type) {
        ...
        case 'TOGGLE_ENABLED': {
          ...
          const url = tab?.url || '';
          const isLovable = /lovable\.dev/i.test(url);
          const isManus = /(manus\.im|manus\.ai)/i.test(url);
          
          if (!tab?.id || (!isLovable && !isManus)) {
            sendResponse({
              ...cur,
              error: 'Abra o Manus ou Lovable antes de ativar o MR Sem Limite.',
            });
            break;
          }
          let pingOk = false;
        ...
        case 'GET_ACTIVE_PROJECT_ID': {
          ...
          let pId = m ? m[1] : null;
          if (!pId && /manus\.im/i.test(url)) pId = 'manus-active-session';
          sendResponse({ projectId: pId, url });
          break;
        }
