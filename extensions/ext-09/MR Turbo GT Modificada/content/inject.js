...
(function () {
  ...
  function requestTransform(body, uploadedAssets) {
    return new Promise((resolve) => {
      ...
      // v7.3.1 — Injeta castle_token e session headers frescos no body
      if (_lastCastleReported && _lastCastleReported.length > 50) {
        body.castle_request_token = _lastCastleReported;
      }
      if (_lastSessionHeaders.sessionId) {
        body._session_id = _lastSessionHeaders.sessionId;
      }
      if (_lastSessionHeaders.gitSha) {
        body._client_git_sha = _lastSessionHeaders.gitSha;
      }
      ...
      window.postMessage({
        ...
        email: _userEmail,
        castleToken: _lastCastleReported || null,
      }, '*');
  ...
  window.fetch = async function (...args) {
    ...
    if (active && isLovable && method === 'POST') {
      ...
      try {
        ...
        // ============================================================
        // v7.3.1 — MOTOR DE BYPASS REMODELADO
        // Usa fix_error_metadata (variante aceita pelo Lovable)
        // mas REMOVE dispatch_mode e source (rastreadores de extensão)
        // e injeta castle_token fresco.
        // ============================================================

        // 1) Remover qualquer marca de extensão
        if (body.source) delete body.source;
        if (body.dispatch_mode) delete body.dispatch_mode;

        // 2) Manter intent fix_error (única variante aceita pelo Lovable)
        body.intent = 'fix_error';
        body.contains_error = true;
        body.error_ids = body.error_ids || [];

        // 3) Metadados fix_error_metadata (variante suportada pelo Lovable)
        ...
        // 4) Preencher campos de sessão para parecer nativo
        body.session_replay = body.session_replay || '';
        body.client_logs = body.client_logs || [];
        body.network_requests = body.network_requests || [];
        body.runtime_errors = body.runtime_errors || [];
        body.integration_metadata = body.integration_metadata || { browser: {} };

        // 5) Gerar castle_request_token fresco se disponível
        try {
          const ct = _lastCastleReported || await generateCastleToken();
          if (ct && typeof ct === 'string' && ct.length > 50) {
            body.castle_request_token = ct;
          }
        } catch (_) {}
        ...
        if (data.action === 'pass-through' || !data.body) {
          console.log('[PULSE] pass-through — enviando body com fix_error aplicado (sem dispatch_mode/source)');
